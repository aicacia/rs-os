...
function createObjectField<
	V extends ObjectSchema<ObjectEntries, ErrorMessage<ObjectIssue> | undefined>
>(schema: V, initialValue: InferInput<V> = {}, debounceMS = 300): ObjectField<V> {
		...
		try {
			...
			await Promise.all(
				Object.entries(fields).map(async ([fieldName, field]) => {
					const [fieldInput, fieldOutput, fieldError] = await field.validate();
...
function createArrayField<
	V extends ArraySchema<
		...
		ErrorMessage<ArrayIssue> | undefined
	>
>(schema: V, initialValue: InferInput<V> = [], debounceMS = 300): ArrayField<V> {
	let state = $state<FieldState>('unset');
	const items = $state(
	...
	function reset(newInitialValue?: InferInput<V>) {
		...
		for (let i = 0; i < items.length; i++) {
			items[i].reset(initialValues[i] as never);
		}
		state = 'unset';
		issues.length = 0;
	}
...
export function createForm<
	V extends ObjectSchema<ObjectEntries, ErrorMessage<ObjectIssue> | undefined>
>(schema: V, initialValue: InferInput<V> = {}, debounceMS = 300) {
