...
pub async fn ensure_oidc_client_exists(
  ...
  let mut model: ActiveModel = ActiveModel::default();

  model.name = Set("OIDC UI".to_string());
  model.client_id = Set(client_id.clone());
  model.auth_method = Set("none".to_string());
  model.application_type = Set("web".to_string());

  model.grant_types = Set(string_vec_to_json(&vec![
    "password".to_string(),
    "authorization_code".to_string(),
    "refresh_token".to_string(),
  ]));
  model.response_types = Set(string_vec_to_json(&vec![
    "code".to_string(),
    "none".to_string(),
  ]));

  model.scopes = Set(string_vec_to_json(&vec![
    "openid".to_string(),
    "profile".to_string(),
    "email".to_string(),
    "address".to_string(),
  ...
  model.audience = Set(string_vec_to_json(&vec![client_id.clone()]));

  model.access_token_expires_in_seconds = Set(config.token.expires_in_seconds as i64);
  model.id_token_expires_in_seconds = Set(config.token.expires_in_seconds as i64);
  model.refresh_expires_in_seconds = Set(config.token.refresh_expires_in_seconds as i64);
