...
use os_model::entities::users::{
  self, assign_user_role, get_user_roles_by_user_id, remove_user_role,
};
...
pub async fn list_user_permissions(
  ...
  for permission_model in permission_models {
    match Permission::from_str(&permission_model.uri) {
      Ok(permission) => {
      ...
      Err(e) => {
        log::error!(
          "error parsing permission uri '{}' to Permission enum: {}",
          permission_model.uri,
          e
        );
        continue;
      }
